...
struct PlanDetailView: View {
    ...
    var body: some View {
        ...
        // Usiamo .sheet per permettere la chiusura tramite scorrimento verso il basso
        .sheet(item: $activeSession) { session in
            WorkoutSessionView(session: session) { saved in
                viewModel.saveSession(saved) { _ in }
            }
            .environmentObject(viewModel)
            .presentationDragIndicator(.visible) // Mostra la lineetta in alto per lo scorrimento
        }
    }
}
